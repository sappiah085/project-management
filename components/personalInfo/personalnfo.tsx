...
// levels
const levels = ["class 2", "Class 1", "class 3", "class 4", "class 5", "class 6", "JHS 1", "JHS 2", "JHS 3"];
export default function PersonalInfo({ setId, _id, handleChangeActive, cookie = "" }: { setId?: (arg: string) => void; handleChangeActive?: any; _id?: string; cookie?: string }) {
  // state values
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit} className="w-full mb-4 pt-9 grid md:grid-cols-2 gap-5">
        <Input name="surname" type="text" label="Surname" placeholder="Kim" value={value.surname} onChange={(e: any) => setInput(e, "surname", setValues)} />

        <Input name="other-name" type="text" label="Other Names" placeholder="Gideon Ofori" value={value.otherNames} onChange={(e: any) => setInput(e, "otherNames", setValues)} />

        <Input name="DOB" type="date" label="Date Of Birth " value={value.DOB} onChange={(e: any) => setInput(e, "DOB", setValues)} placeholder="12-06-2006" />

        <SelectInput value={value.class} setValue={(val: string) => setValues((pre) => ({ ...pre, class: val }))} label="Class or Level" months={levels} />

        <Input name="Nationality" type="text" label="Nationality" value={value.nationality} onChange={(e: any) => setInput(e, "nationality", setValues)} placeholder="Ghanaian" />
        <Input
